...
import { JobPostingJsonLd } from '@/app/components/shared/JsonLd';
...
export async function generateMetadata({
    ...
    try {
        // Fetch job data based on slug
        const job = await getJobItemBySlug(params.slug);

        // If job not found, return minimal metadata
        if (!job) {
            return {
                title: 'Không tìm thấy vị trí tuyển dụng - Minh Minh Tâm',
            };
        }

        // Extract a short description from the job description
        const htmlContent = convertLexicalToHTML({ data: job.description });
        const excerpt = extractExcerpt(htmlContent, 160);

        // Image URL for OpenGraph
        const imageUrl = job.featuredImage?.url
            ? `${process.env.NEXT_PUBLIC_API_URL}${job.featuredImage.url}`
            : `${process.env.NEXT_PUBLIC_BASE_URL}/images/tuyen-dung/banner.png`;

        // Structure the meta description with job details
        const metaDescription = `${job.jobTitle} - ${job.location} - ${excerpt}`;

        return {
            title: `${job.jobTitle} - Tuyển dụng - Minh Minh Tâm`,
            description: metaDescription,
            openGraph: {
                title: job.jobTitle,
                description: metaDescription,
                url: `${process.env.NEXT_PUBLIC_BASE_URL}/${params.locale}/tuyen-dung/${params.slug}`,
                ...
                images: [
                    {
                        ...
                        width: 1200,
                        height: 630,
                        alt: job.featuredImage?.alt || job.jobTitle,
                    },
                ],
            },
            // Add job-specific structured data as JSON-LD
            alternates: {
                canonical: `${process.env.NEXT_PUBLIC_BASE_URL}/${params.locale}/tuyen-dung/${params.slug}`,
            },
        };
    } catch (error) {
        console.error('Error generating metadata for job posting:', error);
        return {
            title: 'Tuyển dụng - Minh Minh Tâm',
        };
    }
}
...
export default async function JobDetail({
    ...
    // Convert job description for structured data
    const descriptionHtml = convertLexicalToHTML({ data: job.description });
    const cleanDescription = extractExcerpt(descriptionHtml, 5000);
    ...
    return (
        <>
            ...
            <JobPostingJsonLd
                ...
                datePosted={new Date().toISOString()} // Use current date since updatedAt is not available
                validThrough={job.applicationDeadline}
                employmentType={job.workType}
                location={job.location}
                baseSiteUrl={process.env.NEXT_PUBLIC_BASE_URL}
                companyLogo={`${process.env.NEXT_PUBLIC_BASE_URL}/images/home/logo.png`}
            />
            <ContentPageLayout
